...
.wlc-top{display:flex;justify-content:flex-end;margin-bottom:4px;}
...
let welcomeStep=0;
function welcomeNext(){ welcomeStep=1; render(); }
...
function viewWelcome(){
  ...
  const dots=[0,1].map(i=>`<span class="wdot ${i===step?'on':''}"></span>`).join('');
  const body = step===0
    ? `<div class="about-fam">${IC.logo}</div><div class="big">${t('wlcTitle1')}</div><p class="wlc-p">${t('wlcBody1')}</p>`
    : `<div class="wlc-ic">${IC.lock}</div><div class="big">${t('wlcTitle2')}</div><p class="wlc-p">${t('wlcBody2')}</p>`;
  return `
    <div class="screen active">
      <div class="wlc-top"><button class="langpill" onclick="toggleLang()">${lang==='FR'?'EN · English':'FR · Français'}</button></div>
      <div class="wlc-wrap">${body}<div class="wdots">${dots}</div></div>
      <button class="btn-primary" onclick="${step===0?'welcomeNext()':'finishWelcome()'}">${step===0?t('wlcNext'):t('wlcStart')}</button>
    </div>`;
}
function viewEditor(){
...
function garageScene(p){
  ...
  for(var i=0;i<3;i++){
    ...
    if(vid && ownsVehicle(p,vid)){
      ...
      // dashed prompt
      svg+='<rect x="'+(x+14)+'" y="62" width="'+(bayW-28)+'" height="50" rx="8" fill="none" stroke="#FF6F61" stroke-width="2" stroke-dasharray="5 4"/>';
      svg+='<text x="'+cx+'" y="91" text-anchor="middle" font-family="Fredoka,sans-serif" font-size="13" font-weight="600" fill="#FF6F61">'+esc(t('garageShopHint'))+'</text>';
    }
  }
  svg+='</svg>';
  return svg;
